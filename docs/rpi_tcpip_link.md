# longer range TCP/IP via NRF24L01 for a pair of Raspberry PIs

## Intro

This tutorial is trying to use `NRF24L01` to create a TCP/IP link between two Raspberry Pi boards.
Due to long range of NRFs, some of those have 1800 meter wireless range, it would be good to have a TCP/IP link between two RPi via NRFs so it would be possible to have a TCP/IP connection between two RPi in longer range. The onboard wifi of RPi cannot do long ranges like 50 meter even in clear sight.

![schematics](rpi_tcpip_link.png)

## Hardware Configuration

1. **Wiring:** Here is how I did connect the module to the RPi as described in the [RF24 documentation](https://nrf24.github.io/RF24/md_docs_2rpi__general.html#autotoc_md268).
...
2. Install nRF24 library stack on each machine. See more detail in the [RF24 documentation](https://nrf24.github.io/RF24/md_docs_2linux__install.html).
...
Installer will prompt which modules you want to install. I did installed all modules: "RF24", "RF24Network" "RF24Mesh" "RF24Gateway". Also please select `SPIDEV` driver during installation.
